...
String text0, text1, text2, text3, text4,text5,text6,text7,text8;//画面出力用
PImage img;//保存用
PImage img2;//保存用
...
String  csv_name    ="Karin_text_products_info.csv";//読み込ませるcsvファイルの名前
...
int Size_X=930, Size_Y=520;        //保存するpngファイルのサイズ(X=ヨコ、Y=タテ)
//設定ここまで
...
void draw() {
  ...
  if (num < lines.length) {
    ...
    if(list.length>=6){
      text(text5, 0, font_size*4+tyousei2*4+tyousei);//保存するテキストを画面に表示する(表示されたテキストが保存される)
    }
    if(list.length>=7){
      text(text6, 0, font_size*5+tyousei2*5+tyousei);//保存するテキストを画面に表示する(表示されたテキストが保存される)
    }
    if(list.length>=8){
      text(text7, 0, font_size*6+tyousei2*6+tyousei);//保存するテキストを画面に表示する(表示されたテキストが保存される)
    }
    if(list.length>=9){
      text(text8, 0, font_size*7+tyousei2*7+tyousei);//保存するテキストを画面に表示する(表示されたテキストが保存される)
    }
    
    
    savetext="quiz_before"+text0;
